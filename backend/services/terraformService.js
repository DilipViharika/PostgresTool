 ...
 * Generates Terraform configs and JSON exports from current FATHOM setup.
 */
...
export async function exportAlertRules(pool) {
    try {
        ...
        for (const rule of res.rows) {
            ...
            hcl += `resource "fathom_alert_rule" "${safeId}" {\n`;
...
export async function exportConnectionConfigs(pool) {
    try {
        ...
        for (const conn of res.rows) {
            ...
            hcl += `resource "fathom_database_connection" "${safeId}" {\n`;
...
export async function exportRetentionPolicies(pool) {
    try {
        ...
        for (const policy of res.rows) {
            ...
            hcl += `resource "fathom_retention_policy" "${safeId}" {\n`;
...
export async function exportUserRoles(pool) {
    try {
        ...
        for (const user of res.rows) {
            ...
            hcl += `resource "fathom_user" "${safeId}" {\n`;
...
export async function generateTerraformBundle(pool) {
    ...
    const bundle = `# FATHOM PostgreSQL Monitoring Tool - Terraform Configuration
# Generated: ${timestamp}
# DO NOT EDIT THIS FILE MANUALLY - Regenerate from FATHOM dashboard

terraform {
  required_version = ">= 1.0"
  required_providers {
    fathom = {
      source  = "fathom/postgresql-monitoring"
      version = "~> 1.0"
    }
  }
}
