...
const DemoPostgresTab = ({ tabId }) => {
    ...
    return (
        ...
        >
                ...
                >
                    ...
                    >
                        <GlassCard>
                            ...
                            <DataTable
                                columns={[
                                    { key: 'name', label: 'Table Name', width: '20%' },
                                    { key: 'size_mb', label: 'Size (MB)', width: '15%' },
                                    { key: 'rows', label: 'Rows', width: '20%' },
                                    { key: 'bloat_pct', label: 'Bloat %', width: '12%' },
                                    { key: 'seq_scan', label: 'Seq Scans', width: '16%' },
                                    { key: 'idx_scan', label: 'Index Scans', width: '17%' },
                                ]}
                                rows={topTables.map((t) => ({
                                    name: t.name,
                                    size_mb: t.size_mb.toLocaleString(),
                                    rows: t.rows.toLocaleString(),
                                    bloat_pct: `${t.bloat_pct}%`,
                                    seq_scan: t.seq_scan.toLocaleString(),
                                    idx_scan: t.idx_scan.toLocaleString(),
                                }))}
                            />
                        </GlassCard>
                        ...
                        >
                            <BentoMetric label="Total Size" value="310GB" icon={<HardDrive size={20} />} color="cyan" />
                            <BentoMetric label="Tables" value="142" icon={<Database size={20} />} color="violet" />
                            <BentoMetric label="Indexes" value="298" icon={<Zap size={20} />} color="emerald" />
                            <BentoMetric
                                label="Growth Rate"
                                value="145MB/day"
            ...
            <div>
                <SectionHeader icon={RefreshCw} title="Replication & WAL" />
                <div
                    style={{
                        display: 'flex',
                        flexDirection: 'column',
                ...
                >
                    <GlassCard>
                        ...
                        <DataTable
                            columns={[
                                { key: 'name', label: 'Replica', width: '20%' },
                                { key: 'lag_ms', label: 'Lag (ms)', width: '15%' },
                                { key: 'state', label: 'State', width: '15%' },
                                { key: 'sent_lsn', label: 'Sent LSN', width: '25%' },
                                { key: 'replay_lsn', label: 'Replay LSN', width: '25%' },
                            ]}
                            rows={replicas.map((r) => ({
                                name: r.name,
                                lag_ms: r.lag_ms.toFixed(1),
                                state: r.state,
                                sent_lsn: r.sent_lsn,
                    ...
                    >
                        <BentoMetric label="WAL Rate" value="2.4 MB/s" icon={<TrendingUp size={20} />} color="cyan" />
                        <BentoMetric label="Checkpoints/day" value="48" icon={<RefreshCw size={20} />} color="violet" />
                        <BentoMetric label="Avg Checkpoint" value="12.4s" icon={<Clock size={20} />} color="emerald" />
                        <MetricCard label="Deadlocks" value="1 in 24h" icon={<Lock size={20} />} color="amber" />
                    </div>
                </div>
            </div>

            {/* Section 8: XID Freeze Warning */}
            <div>
                <SectionHeader icon={AlertCircle} title="XID Freeze Warning" />
                <div
                    style={{
                        display: 'grid',
                        gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))',
                ...
                >
                    <GlassCard>
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
                            <ResourceGauge value={90} max={100} label="Freeze Urgency" />
                        </div>
                    </GlassCard>
                    <MetricCard label="Oldest XID Age" value="180M" icon={<AlertCircle size={20} />} color="amber" />
                    <MetricCard label="Freeze Max Age" value="200M" icon={<Gauge size={20} />} color="cyan" />
                </div>
            </div>
        </div>
    );
