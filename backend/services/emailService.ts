...
class EmailNotificationService {
  ...
  private initialize(): void {
    ...
    try {
      ...
      log('ERROR', 'Failed to initialize email service', { error: (error as Error).message });
      this.enabled = false;
    }
  }
  ...
  async verifyConnection(): Promise<boolean> {
    ...
    try {
      ...
      log('WARN', 'Email server connection failed', { error: (error as Error).message });
      return false;
    }
  }
...
<body style="margin:0;padding:0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;background-color:#f3f4f6;">
  ...
  private generatePlainTextEmail(alert: Alert): string {
    let text = `
═══════════════════════════════════════════════════════
...
═══════════════════════════════════════════════════════`;
  ...
  private formatMetricValue(key: string, value: any): string {
    ...
    if (key.includes('Ratio') || key.includes('Pct')) return `${parseFloat(String(value)).toFixed(1)}%`;
    if (key.includes('GB') || key.includes('Size')) return `${parseFloat(String(value)).toFixed(2)} GB`;
    if (key.includes('Seconds') || key.includes('Uptime')) return `${Math.floor(Number(value) / 3600)}h`;
    return String(value);
  }

  async sendAlert(alert: Alert, recipients?: string[]): Promise<SendResult> {
    if (!this.enabled || !this.transporter) {
      return { success: false, reason: 'disabled' };
    }

    const severityLevels: Record<string, number> = { info: 1, warning: 2, critical: 3 };
    ...
    try {
      ...
      log('ERROR', 'Failed to send alert email', { error: (error as Error).message });
      return { success: false, error: (error as Error).message };
    }
  }

  async sendTestEmail(recipient: string): Promise<SendResult> {
  ...
  async sendDigest(alertList: Alert[], recipients?: string[]): Promise<SendResult> {
    if (!this.enabled || !this.transporter) {
      return { success: false, reason: 'disabled' };
    }
...
<body style="margin:0;padding:20px;font-family:Arial,sans-serif;background-color:#f3f4f6;">
    ...
    try {
      ...
      log('ERROR', 'Failed to send digest email', { error: (error as Error).message });
      return { success: false, error: (error as Error).message };
    }
  }
}
