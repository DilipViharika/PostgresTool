...
describe('tabConfig', () => {
  ...
  describe('buildTabConfig()', () => {
    ...
    it('should contain section headers', () => {
      ...
      expect(sections.some(s => s.section === 'Core Monitoring')).toBe(true);
    });
    ...
    it('should include specific key tabs', () => {
      ...
      const expectedTabs = [
        'overview', 'performance', 'resources',
        'optimizer', 'indexes',
        'pool', 'backup',
        'schema', 'security',
        'sql', 'api',
    ...
    it('should attach icons and components to tabs', () => {
      ...
      tabs.forEach(tab => {
        ...
        expect(typeof tab.icon).toBe('function'); // lucide-react components are functions
      });
    });
  });
  ...
  describe('getSectionGroups()', () => {
    ...
    it('should properly nest tabs under their sections', () => {
      ...
      const coreMonitoring = groups.find(g => g.section === 'Core Monitoring');
      expect(coreMonitoring).toBeDefined();
      expect(coreMonitoring.tabs.length).toBeGreaterThan(0);
      expect(coreMonitoring.tabs.some(t => t.id === 'overview')).toBe(true);
    });
  ...
  describe('getSectionForTab()', () => {
    it('should find section for a valid tab', () => {
      ...
      expect(section).toBe('Core Monitoring');
    });
    ...
    it('should correctly identify tabs in different sections', () => {
      ...
      expect(getSectionForTab(groups, 'overview')).toBe('Core Monitoring');
      expect(getSectionForTab(groups, 'optimizer')).toBe('Query & Indexes');
