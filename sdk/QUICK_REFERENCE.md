# VIGIL SDK Quick Reference Guide
...
### 1. VigilClient - Main API Interface
...
// Sync to VIGIL
await sf.syncToVigil(client, { includeAuditTrail: true });
```
...
```javascript
const client = new VigilClient({ /* config */ });
...
const usagePercent = (metrics.limits.ApiCalls.Used /
                      metrics.limits.ApiCalls.Max) * 100;
...
// Sync to VIGIL dashboard
await sf.syncToVigil(client);
```
...
const metrics = await Promise.all([
  postgres.getPerformance(),
  mysql.getPerformance(),
  mongo.getPerformance()
]);
...
```javascript
const client = new VigilClient({
  baseUrl: process.env.VIGIL_API_URL,
  apiKey: process.env.VIGIL_API_KEY,
  orgId: process.env.VIGIL_ORG_ID,
  options: {
    logLevel: 'warn',
    retryAttempts: 5,
    timeout: 60000
...
2. **Batch Operations**: Use Promise.all() for parallel requests
   ```javascript
   const [sf, mule, db] = await Promise.all([
     sf.getMetrics(),
     mule.getApps(),
     db.getOverview()
   ]);
