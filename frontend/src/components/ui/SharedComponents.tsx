...
export const GlassCard: FC<GlassCardProps> = ({ children, title }) => <div style={{ background: THEME.glass, borderRadius: 4, border: `1px solid ${THEME.border}`, padding: 20 }}>{title && <h3 style={{ margin: 0, fontSize: 12, fontWeight: 700 }}>{title}</h3>}{children}</div>;
...
export const CustomTooltip: FC<CustomTooltipProps> = ({ active, payload, label }) => active && payload ? <div style={{ background: THEME.surface, padding: 8, borderRadius: 4, fontSize: 11 }}><p>{label}</p>{payload.map((p, i) => <div key={i}>{p.name}: {p.value}</div>)}</div> : null;
...
export const SkeletonLoader: FC<SkeletonLoaderProps> = ({ rows = 3, height = 16, gap = 10, variant = 'line' }) => variant === 'card' ? <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 12 }}>{Array.from({ length: rows }).map((_, i) => <div key={i} style={{ height: 120, borderRadius: 4, background: 'rgba(255,255,255,0.03)', border: `1px solid ${THEME.border}` }} />)}</div> : <div style={{ display: 'flex', flexDirection: 'column', gap }}>{Array.from({ length: rows }).map((_, i) => <div key={i} style={{ height, borderRadius: 2, background: THEME.grid, width: i === rows - 1 ? '55%' : '100%' }} />)}</div>;

export const LoadingOverlay: FC<{ message?: string }> = ({ message }) => <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,2,12,0.8)', zIndex: 20, borderRadius: 4, gap: 12 }}><div style={{ width: 40, height: 40, borderRadius: '50%', border: `2px solid ${THEME.border}`, borderTop: `2px solid ${THEME.primary}`, animation: 'spin 0.8s linear infinite' }} />{message && <span style={{ fontSize: 10, color: THEME.textMuted, fontFamily: THEME.fontMono }}>{message}</span>}</div>;
...
export const StatCompare: FC<StatCompareProps> = ({ label, before, after, unit }) => <div style={{ background: 'rgba(4,9,28,0.7)', borderRadius: 4, padding: 16, border: `1px solid ${THEME.border}` }}><div style={{ fontSize: 9, color: THEME.textMuted, fontFamily: THEME.fontDisplay, marginBottom: 12 }}>{label}</div><div style={{ display: 'flex', alignItems: 'flex-end', gap: 16 }}><div><div style={{ fontSize: 9, color: THEME.textDim, marginBottom: 3 }}>BEFORE</div><span style={{ fontSize: 22, fontWeight: 700, color: THEME.textMuted, fontFamily: THEME.fontMono }}>{before}{unit && <span style={{ fontSize: 11 }}> {unit}</span>}</span></div><div style={{ fontSize: 16, color: THEME.textDim }}>→</div><div><div style={{ fontSize: 9, color: THEME.textDim, marginBottom: 3 }}>AFTER</div><span style={{ fontSize: 22, fontWeight: 800, color: THEME.textMain, fontFamily: THEME.fontMono }}>{after}{unit && <span style={{ fontSize: 11, color: THEME.textMuted }}> {unit}</span>}</span></div></div></div>;

// Terminal and input components
export const Terminal: FC<TerminalProps> = ({ lines = [], title = 'neural://shell', onExecute, readOnly = false, maxHeight = 300 }) => <div style={{ background: '#01060e', borderRadius: 4, border: `1px solid ${THEME.border}`, overflow: 'hidden', display: 'flex', flexDirection: 'column', maxHeight, fontFamily: THEME.fontMono, fontSize: 11 }}><div style={{ padding: '8px 12px', borderBottom: `1px solid ${THEME.border}`, color: THEME.textMuted }}>{title}</div><div style={{ flex: 1, overflow: 'auto', padding: 12 }}>{lines.map((line, i) => <div key={i} style={{ color: THEME.textMuted, marginBottom: 4 }}>{line.content}</div>)}</div></div>;
...
export const CommandPaletteItem: FC<CommandPaletteItemProps> = ({ icon: Icon, label, description, shortcut, color = THEME.primary, onClick, active }) => {
    ...
    return <div {...hoverProps} onClick={onClick} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', borderRadius: 3, background: isHighlighted ? `${color}08` : 'transparent', border: `1px solid ${isHighlighted ? color + '22' : 'transparent'}`, borderLeft: `2px solid ${isHighlighted ? color : 'transparent'}`, cursor: 'pointer', transition: 'all 0.15s' }}>{Icon && <div style={{ width: 30, height: 30, borderRadius: 4, background: `${color}12`, color, display: 'flex', alignItems: 'center', justifyContent: 'center', border: `1px solid ${color}20`, flexShrink: 0 }}><Icon size={13} /></div>}<div style={{ flex: 1 }}><div style={{ fontSize: 12, color: isHighlighted ? THEME.textMain : THEME.textMuted, fontFamily: THEME.fontBody, fontWeight: 500 }}>{label}</div>{description && <div style={{ fontSize: 10, color: THEME.textDim, marginTop: 1 }}>{description}</div>}</div>{shortcut && <kbd style={{ fontSize: 9, color: THEME.textDim, background: 'rgba(255,255,255,0.05)', border: `1px solid ${THEME.border}`, padding: '2px 7px', borderRadius: 3, fontFamily: THEME.fontMono }}>{shortcut}</kbd>}</div>;
};
