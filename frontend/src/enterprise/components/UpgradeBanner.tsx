...
const UpgradeBanner: React.FC<UpgradeBannerProps> = ({
  ...
  const tierDisplay = (tier?.charAt(0).toUpperCase() || 'P') + (tier?.slice(1) || 'ro');

  const styles: ThemeColors = isDark
    ? {
        bg: '#0d1224',
  ...
  return (
    <div style={containerStyle}>
      ...
      <div style={contentStyle}>
        <p style={titleStyle}>{featureName} is available in {tierDisplay} and above</p>
        <p style={descStyle}>{featureDesc}</p>
      </div>
      <button
