...
// License signatures are truncated to 32 hex chars (128 bits) — long enough
// that brute-force forgery is infeasible. Older keys signed with the previous
// 16-hex (64-bit) truncation are still accepted by length, but new keys
// generated by this codepath always use the longer form.
const LICENSE_SIG_LEN_NEW = 32;
const LICENSE_SIG_LEN_LEGACY = 16;
...
export function validateLicenseKeyFormat(licenseKey, licenseSecret) {
  ...
  // Pick truncation length to match the key being verified so timingSafeEqual
  // can run over equal-length buffers. Anything that isn't one of our accepted
  // lengths is rejected outright.
  const sigLen = providedSig.length;
  if (sigLen !== LICENSE_SIG_LEN_NEW && sigLen !== LICENSE_SIG_LEN_LEGACY) {
    return { valid: false, error: 'License key signature is invalid' };
  }

  // Verify signature with a constant-time comparison so an attacker cannot
  // recover the expected HMAC byte-by-byte via timing differences.
  ...
  if (providedBuf.length === expectedBuf.length) {
    sigOk = crypto.timingSafeEqual(providedBuf, expectedBuf);
  }
