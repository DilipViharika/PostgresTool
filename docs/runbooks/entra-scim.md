# Microsoft Entra ID SCIM Provisioning for VIGIL

**Audience:** Entra admin at a VIGIL Enterprise tenant.
**Prerequisite:** Enterprise plan; a VIGIL admin account; tenant ID for
...
- Assignment of a user to the VIGIL enterprise app provisions them.
- Removal or disablement in Entra deactivates them in VIGIL
  (`active: false`).
...
4. Copy the token (shown once; starts with `vigil_scim_`) into your
...
5. Admin Credentials:
    - **Tenant URL**: `https://<your-vigil-host>/scim/v2`
    - **Secret Token**: paste the `vigil_scim_...` value.
...
Entra's default mappings; adjust to match VIGIL's expectations:

| Entra attribute                                             | SCIM (VIGIL) attribute         | Notes                                                   |
| ----------------------------------------------------------- | ------------------------------ | ------------------------------------------------------- |
...
| `objectId`                                                  | `externalId`                   | Stable across renames; VIGIL uses this for correlation. |

Turn off any mappings for attributes VIGIL doesn't accept (timezone,
...
1. After the first sync cycle (or after an on-demand run), open VIGIL
   → Settings → Users. Your assigned users should be present.
2. Remove an assignment in Entra for a test user. The next sync
   should mark them `active: false` in VIGIL within ~40 minutes.
3. Audit tab in VIGIL shows the SCIM events (`CREATE_USER`,
...
- Rotate every 90 days. Procedure:
    1. Mint a new token in VIGIL.
    2. In Entra, update Admin Credentials → Secret Token.
    3. Test Connection.
    4. Revoke the old token in VIGIL.
- Emergency revocation: VIGIL → Settings → SCIM tokens → Revoke. The
...
| Provisioning shows "skipped — no matching SCIM attribute"        | Remove the offending mapping entry; VIGIL only supports the attributes in the table above.                           |
...
- If you rotate Entra's tenant (e.g. migrate from test to prod), mint
  a fresh SCIM token in the new VIGIL tenant; do **not** re-use the
  test token.
