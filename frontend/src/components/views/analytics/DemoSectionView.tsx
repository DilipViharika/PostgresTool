...
interface DemoSectionViewProps {
    tabId?: string;
}

const DB_MAP: { [key: string]: string } = {
    pg: 'postgresql',
    mysql: 'mysql',
    mongo: 'mongodb',
};
...
const SQL_TAB_TO_SECTION: { [key: string]: string } = {
    // Core Monitoring
...
const DemoSectionView: React.FC<DemoSectionViewProps> = ({ tabId: propTabId }) => {
    // Prefer prop-based tabId (passed from App.tsx) over localStorage for reliable re-renders
    const tabId = propTabId || (() => {
        try {
            return localStorage.getItem('pg_monitor_active_tab') || '';
        } catch {
            return '';
        }
    })();
...
export default DemoSectionView;
