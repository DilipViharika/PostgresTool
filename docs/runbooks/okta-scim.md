# Okta SCIM Provisioning for VIGIL

**Audience:** IT admin or identity engineer at a VIGIL Enterprise tenant.
**Prerequisite:** Your VIGIL tenant is on the Enterprise plan (SCIM is a
gated feature). You have at least one VIGIL admin account.
...
- When a user is assigned to the VIGIL app in Okta, they appear in
  VIGIL with the right attributes.
- When a user is un-assigned or deactivated in Okta, they are
  deactivated in VIGIL (SCIM `active: false`).
- Group memberships pushed from Okta map to VIGIL roles.

No passwords cross the wire; authentication to VIGIL is still handled
via SAML SSO (`sso_saml` feature), which you configure separately.

---

## Step 1 — Mint a SCIM bearer token in VIGIL

1. Sign in to VIGIL as an admin.
2. Settings → Integrations → SCIM provisioning.
3. Click **Create token**. Label it `okta-prod` (or per-environment).
4. Copy the token — it's shown **once**. It starts with `vigil_scim_`.
5. Paste it into your password manager. You cannot recover it later;
   if you lose it, revoke it and mint a new one.

Behind the scenes, VIGIL stores only `SHA-256(token)`. The token itself
never touches disk in cleartext.

---

## Step 2 — Add the VIGIL app in Okta

1. Okta Admin → Applications → Browse App Catalog.
2. If a VIGIL entry exists, use it. Otherwise choose **Create New App →
...
4. Fill in:
    - **Base URL**: `https://<your-vigil-host>/scim/v2`
    - **API Token**: paste the `vigil_scim_...` token from Step 1.
...
- Your VIGIL tenant is Enterprise — a 403 with `feature_not_available`
...
Okta → VIGIL SCIM Core 2.0 user schema. These map by default; verify on
...
| `id`           | `externalId`            | Okta user id; lets VIGIL correlate after username changes. |
...
| Okta group      | VIGIL role |
| --------------- | ---------- |
| `VIGIL-Admins`  | `admin`    |
| `VIGIL-DBAs`    | `dba`      |
| `VIGIL-Viewers` | `viewer`   |

VIGIL expects the group `displayName` to match one of the role names
...
3. In VIGIL → Settings → Users, confirm the users appear.
4. Un-assign one test user in Okta → confirm they show as inactive in
   VIGIL within a minute.
...
- To rotate: mint a new token in VIGIL, update Okta's API Integration
...
| `403 feature_not_available`    | Tenant dropped off Enterprise. Contact your VIGIL CSM.                                                                      |
...
| 5xx responses                  | Check VIGIL status page; if persistent, open a support ticket with the `X-Request-Id` header from the failed SCIM response. |
...
- All SCIM operations are written to the VIGIL tamper-evident audit
