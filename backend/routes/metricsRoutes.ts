...
export default function metricsRoutes(pool: Pool, authenticate: any): Router {
    const router = Router();

    // ── GET /metrics/registry ─ Return metric definitions for active connection's DB type ───
    router.get('/metrics/registry', authenticate, async (req: Request, res: Response) => {
        try {
            // Default to PostgreSQL; in a full implementation, fetch from session context
            const dbType = req.query.dbType || 'postgresql';
            const metrics = getMetricsForDbType(dbType as string);
    ...
    router.get('/metrics/registry/:dbType', authenticate, (req: Request, res: Response) => {
        try {
    ...
    router.get('/metrics/categories', authenticate, (req: Request, res: Response) => {
        try {
    ...
    router.get('/metrics/current', authenticate, async (req: Request, res: Response) => {
        try {
            const dbType = req.query.dbType || 'postgresql';
            const metrics = getMetricsForDbType(dbType as string);
            ...
            if (!pool) {
                ...
                metrics.forEach((m: any) => {
                    ...
                    const health = evaluateMetricHealth(m.id, mockValue, dbType as string);
                    current[m.id] = {
                ...
                for (const metric of metrics) {
                    try {
                        ...
                        const health = evaluateMetricHealth(metric.id, value, dbType as string);
    ...
    router.get('/metrics/history/:metricId', authenticate, async (req: Request, res: Response) => {
        try {
            ...
            const now = new Date();
            const history = Array.from({ length: Math.min(parseInt(hours as string), 24) }, (_, i) => {
                const timestamp = new Date(now.getTime() - (parseInt(hours as string) - i - 1) * 3600000);
            ...
            const metric = getMetricById(metricId, dbType as string);
            if (!metric) {
    ...
    router.get('/metrics/health', authenticate, async (req: Request, res: Response) => {
        try {
            const dbType = req.query.dbType || 'postgresql';
            const metrics = getMetricsForDbType(dbType as string);
            const health: Record<string, number> = { ok: 0, warning: 0, critical: 0, unknown: 0 };

            metrics.forEach((m: any) => {
                const mockValue = Math.random() * 100;
                const h = evaluateMetricHealth(m.id, mockValue, dbType as string);
                health[h.status]++;
            });
    ...
    router.get('/metrics/by-category/:category', authenticate, (req: Request, res: Response) => {
        try {
            const { category } = req.params;
            const dbType = req.query.dbType || 'postgresql';
            const metrics = getMetricsByCategory(category, dbType as string);
