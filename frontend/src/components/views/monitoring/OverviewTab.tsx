...
import {
    ...
    XAxis, YAxis, Tooltip, CartesianGrid, LineChart, Line, ReferenceLine,
} from 'recharts';

import {
    PanelCard, KpiCard, RingGauge, StatusBadge, LiveDot, Divider, ChartTooltip,
    ProgressBar, StatStrip, MiniStat, TremorStyles,
} from '../../ui/tremor';
...
const MiniSparkline = React.memo(({ data = [], color = THEME.primary, width = 64, height = 20, filled = true }) => {
    ...
    const pts = data
        ...
        .join(' ');
    const uid = `sp-${color.replace(/[^a-z0-9]/gi, '')}-${width}`;
    return (
        ...
        <div ref={ref} style={{ position: 'relative' }}>
            <button
                ...
                style={{
                    ...
                    background: `${env.color}12`, border: `1px solid ${env.color}28`, cursor: 'pointer', color: env.color,
            ...
            {open && (
                <div
                    style={{
                        ...
                        background: THEME.surfaceRaised, border: `1px solid ${THEME.glassBorder}`, borderRadius: 12,
                        overflow: 'hidden', minWidth: 240, boxShadow: '0 20px 40px rgba(0,0,0,0.4)',
                    }}
                >
                    <div style={{ padding: '8px 12px 6px', borderBottom: `1px solid ${THEME.glassBorder}` }}>
                        <span style={{ fontSize: 9.5, fontWeight: 700, color: THEME.textDim, letterSpacing: '0.02em' }}>
                            Switch Environment
                        </span>
                    </div>
                    {ENVIRONMENTS.map((e) => (
                        <button
                            ...
                            style={{
                                ...
                                background: e.id === currentEnv ? `${e.color}10` : 'transparent', border: 'none',
                                borderBottom: `1px solid ${THEME.glassBorder}20`, cursor: 'pointer', transition: 'background 0.15s',
                            }}
                        ...
                        >
                            <div style={{
                                width: 32, height: 32, borderRadius: 10, background: `${e.color}16`, border: `1px solid ${e.color}28`,
                    ...
                    <div style={{ padding: '6px 12px 8px', display: 'flex', alignItems: 'center', gap: 6 }}>
                        ...
                        <span style={{ fontSize: 9.5, color: THEME.textDim }}>
                            {ENVIRONMENTS.length > 0 ? `${ENVIRONMENTS.length} environments configured` : 'No environments'}
                        </span>
                    </div>
                </div>
            )}
...
const NotificationBell = React.memo(() => {
    ...
    return (
        <div ref={ref} style={{ position: 'relative' }}>
            ...
            >
                {unread > 0 ? (
                    <BellRing size={15} color={SEVERITY_COLOR.critical} style={{ animation: bellAnim ? 'tremorBell 1.2s ease-in-out' : 'none' }} />
                ) : (
                ...
                {unread > 0 && (
                    <span style={{
                        ...
                        padding: '0 4px', background: SEVERITY_COLOR.critical, color: '#fff', fontSize: 9, fontWeight: 800,
                        display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: THEME.fontMono,
            ...
            {open && (
                <div style={{
                    ...
                    background: THEME.surfaceRaised, border: `1px solid ${THEME.glassBorder}`, borderRadius: 12,
                    overflow: 'hidden', width: 320, boxShadow: '0 20px 40px rgba(0,0,0,0.4)',
                }}>
                    <div style={{
                        padding: '10px 14px', borderBottom: `1px solid ${THEME.glassBorder}`,
                        ...
                        {unread > 0 && (
                            <button
                                onClick={markAllRead}
                                style={{ fontSize: 10, color: THEME.primary, background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}
                            >
                                Mark all read
                            </button>
                        )}
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        <div style={{
                                            fontSize: 11.5, color: a.read ? THEME.textMuted : THEME.textMain,
                                            fontWeight: a.read ? 400 : 600, lineHeight: 1.35,
                                        }}>
                                            {a.title}
                                        </div>
                                    ...
                                    <button
                                        ...
                                        style={{ background: 'none', border: 'none', cursor: 'pointer', color: THEME.textDim, padding: 0, opacity: 0.6 }}
                                    >
                                        <X size={11} />
                                    </button>
                                </div>
                    ...
                    <div style={{
                        padding: '8px 14px', borderTop: `1px solid ${THEME.glassBorder}`, display: 'flex', justifyContent: 'center',
                    }}>
        ...
        <div ref={ref} style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 6 }}>
            <button
                ...
                style={{
                    ...
                    border: `1px solid ${THEME.glassBorder}`, cursor: loading ? 'not-allowed' : 'pointer',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s', opacity: loading ? 0.5 : 1,
                }}
            >
            ...
            <button
                ...
                style={{
                    display: 'flex', alignItems: 'center', gap: 5, padding: '6px 12px', borderRadius: 10, height: 32,
                    background: interval === 0 ? 'rgba(239,68,68,0.1)' : 'rgba(255,255,255,0.05)',
                    border: `1px solid ${interval === 0 ? '#ef444440' : THEME.glassBorder}`,
                    cursor: 'pointer', color: interval === 0 ? '#ef4444' : THEME.textMuted, transition: 'all 0.15s',
            ...
            {open && (
                <div style={{
                    ...
                    background: THEME.surfaceRaised, border: `1px solid ${THEME.glassBorder}`, borderRadius: 12,
                    overflow: 'hidden', minWidth: 140, boxShadow: '0 20px 40px rgba(0,0,0,0.4)',
                }}>
                    <div style={{ padding: '7px 11px 5px', borderBottom: `1px solid ${THEME.glassBorder}` }}>
                        <span style={{ fontSize: 9, fontWeight: 700, color: THEME.textDim, letterSpacing: '0.02em' }}>
                            Refresh interval
                        </span>
                    </div>
                    {REFRESH_OPTIONS.map((opt) => (
                        <button
                            ...
                            style={{
                                ...
                                border: 'none', borderBottom: `1px solid ${THEME.glassBorder}15`, cursor: 'pointer', transition: 'background 0.12s',
                            }}
                            onMouseEnter={(ev) => {
                                if (opt.value !== interval) ev.currentTarget.style.background = 'rgba(255,255,255,0.04)';
                            }}
                            onMouseLeave={(ev) => {
...
const BackupStatusCard = ({ lastBackup }) => {
    ...
    return (
        <div style={{
            ...
            background: THEME.surface, border: `1px solid ${isOld ? `${THEME.warning}30` : THEME.glassBorder}`,
            position: 'relative', overflow: 'hidden', boxShadow: THEME.shadowSm,
        }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div style={{
                        width: 32, height: 32, borderRadius: 10, background: `${statusColor}14`,
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                    }}>
            ...
            <div style={{
                display: 'flex', alignItems: 'center', gap: 5, paddingTop: 8,
                ...
                {isOld && <span style={{ marginLeft: 'auto' }}><StatusBadge label="OVERDUE" color={THEME.warning} /></span>}
            </div>
        </div>
    );
};
...
const LongTxnCard = ({ data, onNavigate }) => {
    ...
    if (txns.length === 0) {
        return (
            <div style={{
                padding: '16px 18px', borderRadius: 12, background: THEME.surface,
                border: `1px solid ${THEME.glassBorder}`, position: 'relative', overflow: 'hidden',
                boxShadow: '0 0 0 1px rgba(0,0,0,0.04), 0 4px 12px rgba(0,0,0,0.08)',
            }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <div style={{
                            width: 32, height: 32, borderRadius: 10, background: `${THEME.success}14`,
                            border: `1px solid ${THEME.success}28`, display: 'flex', alignItems: 'center', justifyContent: 'center',
                        }}>
    ...
    return (
        <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div style={{
                        width: 32, height: 32, borderRadius: 10, background: `${THEME.warning}14`,
                        border: `1px solid ${THEME.warning}28`, display: 'flex', alignItems: 'center', justifyContent: 'center',
                    }}>
                ...
                {txns.slice(0, 3).map((t, i) => {
                    ...
                    return (
                        <div key={t.pid} style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                            ...
                            <div style={{
                                fontSize: 10, color: THEME.textDim, whiteSpace: 'nowrap', overflow: 'hidden',
...
const VacuumHealthCard = ({ data, onNavigate }) => {
    ...
    return (
        <div style={{
            ...
            boxShadow: vacuum.urgentCount > 3
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                ...
                <div style={{ textAlign: 'right' }}>
                    ...
                    <div style={{
                        fontSize: 9, color: THEME.textDim, fontWeight: 600, letterSpacing: '0.04em',
                    }}>
                        urgent
                    </div>
                </div>
            ...
            <div style={{
                height: 7, borderRadius: 14, background: `${THEME.grid}50`, overflow: 'hidden',
                display: 'flex', marginBottom: 8, boxShadow: 'inset 0 1px 2px rgba(0,0,0,0.2)',
            }}>
                <div style={{
                    width: `${urgentPct}%`, height: '100%', background: THEME.danger, transition: 'width 0.9s ease',
                }} />
                <div style={{
                    width: `${warnPct}%`, height: '100%', background: THEME.warning, transition: 'width 0.9s ease 0.05s',
                }} />
                <div style={{
                    flex: 1, minWidth: 0, height: '100%', background: `${THEME.success}60`, transition: 'width 0.9s ease 0.1s',
                }} />
            </div>

            <div style={{
                display: 'flex', justifyContent: 'space-between', fontSize: 10, color: THEME.textDim, marginBottom: 10,
            }}>
...
const ConnectionStatusBanner = () => {
    ...
    if (!loading && connections.length === 0) {
        return (
            <div style={{
                borderRadius: 16, background: `linear-gradient(135deg, ${THEME.primary}10, ${THEME.secondary || THEME.primary}08)`,
                border: `1px dashed ${THEME.primary}50`, padding: '28px 32px', display: 'flex', alignItems: 'center', gap: 24,
            }}>
                <div style={{
                    width: 64, height: 64, borderRadius: 16, flexShrink: 0, background: `${THEME.primary}15`,
                    border: `1px solid ${THEME.primary}30`, display: 'flex', alignItems: 'center', justifyContent: 'center',
                }}>
                    ...
                    <div style={{ marginTop: 14, display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                        {[
                            ...
                            <span key={label} style={{
                                display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 11, color: THEME.textMuted,
                ...
                <button
                    onClick={goToPool}
                    style={{
                        flexShrink: 0, padding: '11px 22px', background: `linear-gradient(135deg, ${THEME.primary}, ${THEME.secondary || THEME.primary})`,
                        border: 'none', borderRadius: 10, cursor: 'pointer', color: '#fff', fontSize: 13, fontWeight: 700,
                        letterSpacing: '0.02em', boxShadow: `0 4px 18px ${THEME.primary}40`,
                        display: 'flex', alignItems: 'center', gap: 8, transition: 'transform 0.12s, box-shadow 0.12s',
                    }}
                    onMouseEnter={(e) => {
    ...
    const statusColor = activeConnection.status === 'success' ? THEME.success : activeConnection.status === 'failed' ? THEME.danger : THEME.warning;
    ...
    return (
        <div style={{
            borderRadius: 14, background: THEME.surface, padding: '12px 16px',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0, flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    <span style={{
                        fontSize: 10, color: statusColor, fontWeight: 600, letterSpacing: '0.02em'
                    }}>
                        ● {activeConnection.status || 'untested'}
                    </span>
                    {activeConnection.ssl && (
                        <span style={{
                            fontSize: 10, color: THEME.success, background: `${THEME.success}18`,
                    ...
                    {activeConnection.sshEnabled && (
                        <span style={{
                            fontSize: 10, color: '#818cf8', background: '#818cf818',
                            padding: '1px 7px', borderRadius: 20, fontFamily: 'monospace',
                        }}>
                            SSH
                        </span>
                        ...
                        .map((c) => (
                            <button
                                ...
                                style={{
                                    ...
                                    border: `1px solid ${THEME.primary}30`, color: THEME.primary, fontWeight: 500,
            ...
            <button
                onClick={goToPool}
                style={{
                    flexShrink: 0, padding: '5px 12px', borderRadius: 7, cursor: 'pointer', background: 'transparent',
                    border: `1px solid ${THEME.glassBorder}`, color: THEME.textMuted, fontSize: 11, display: 'flex',
        ...
        return (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20, padding: '0 0 40px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 12 }}>
                    {[0, 1, 2, 3, 4, 5].map((i) => (
                        <div
                            key={i}
                            style={{
                                height: 110, borderRadius: 14, background: THEME.surface, border: `1px solid ${THEME.glassBorder}`,
                                opacity: 0.4, animation: `tremorPulse 1.5s ease-in-out infinite`, animationDelay: `${i * 0.1}s`,
                            }}
                        />
                    ))}
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 18 }}>
                    <div style={{
                        height: 320, borderRadius: 16, background: THEME.surface, border: `1px solid ${THEME.glassBorder}`,
                        opacity: 0.2, animation: 'tremorPulse 1.5s ease-in-out infinite 0.5s',
                    }} />
                    <div style={{
                        height: 320, borderRadius: 16, background: THEME.surface, border: `1px solid ${THEME.glassBorder}`,
                        opacity: 0.2, animation: 'tremorPulse 1.5s ease-in-out infinite 0.65s',
                    }} />
                </div>
            </div>
        );
        ...
        return (
            <div style={{ padding: 40, textAlign: 'center' }}>
                ...
                <button
                    onClick={() => load(true)}
                    style={{
                        padding: '8px 20px', borderRadius: 8, border: `1px solid ${THEME.glassBorder}`,
                        background: THEME.surface, color: THEME.textMain, cursor: 'pointer', fontSize: 13,
                    }}
                >
                    Retry
                </button>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20, padding: '12px 0 40px 0' }}>
            ...
            <div className="ov-stagger" style={{ display: 'grid', gridTemplateColumns: '1.7fr 1fr', gap: 18 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                    {/* Health Score */}
                    <PanelCard title="Database Health" icon={Shield} accentColor={healthColor}>
                            ...
                            <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', gap: 8 }}>
                                {[
                                    ...
                                    <div key={i}>
                                        ...
                                        <ProgressBar value={Math.min(item.pct, 100)} color={item.color} height={5} showLabel={false} />
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    {/* Connection Pool */}
                    <PanelCard title="Connection Pool" icon={Network} accentColor={connColor}>
            ...
            <PanelCard title="Transaction Latency Percentiles" icon={Gauge} accentColor={THEME.secondary}
                rightNode={
            ...
            <div className="ov-stagger" style={{ display: 'grid', gridTemplateColumns: '1fr 1.3fr 1.3fr', gap: 18 }}>
                ...
                <PanelCard title="Throughput Breakdown" icon={TrendingUp} noPadding>
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                        {throughputRows.map((row, i) => {
                            const pct = (row.raw / maxThroughput) * 100;
                            return (
                                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 18px', borderBottom: i < throughputRows.length - 1 ? `1px solid ${THEME.grid}22` : 'none' }}>
                                    <div style={{ width: 32, height: 32, borderRadius: 10, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: `${row.color}12`, border: `1px solid ${row.color}20` }}>
                                    ...
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        ...
                                        <ProgressBar value={pct} color={row.color} height={4} showLabel={false} animate />
                                    </div>
                                </div>
                            );
                        })}
                ...
                {/* Ops/sec */}
                <PanelCard title="Ops / Second (today)" icon={Gauge}
                    rightNode={
            ...
            <div className="ov-stagger" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 18 }}>
                {[
                    ...
                    { label: 'Disk I/O', value: Math.round((diskGB / 200) * 100), color: THEME.warning, icon: HardDrive, detail: `${diskGB} GB used`, status: diskGB > 160 ? 'High' : 'Normal', secondary: 0, secondaryLabel: 'Write Amp' },
                ].map((r, i) => (
                    <PanelCard key={i} accentColor={r.color}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div className="ov-stagger" style={{ display: 'grid', gridTemplateColumns: '1.4fr 1.4fr 1.2fr', gap: 18 }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                            ...
                            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                                {(replicationData?.replicas || []).length > 0 ? (replicationData.replicas || []).map((r, i) => (
                                    <div key={i} style={{ padding: '8px 12px', borderRadius: 10, background: THEME.surface, border: `1px solid ${r.lagMs > 300 ? `${THEME.warning}40` : `${THEME.grid}50`}`, minWidth: 112, boxShadow: 'inset 0 1px 2px rgba(255,255,255,0.08)' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 1fr', gap: 12, alignItems: 'center' }}>
                            <div>
                                ...
                                <ProgressBar value={replicationData?.lockContention ? Math.min(replicationData.lockContention, 100) : 0} color={THEME.danger} height={5} showLabel={false} animate={false} />
                ...
                {/* Top Impacted Tables */}
                <PanelCard title="Top Impacted Tables" icon={BarChart3} noPadding>
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                {/* WAL & Checkpoints */}
                <PanelCard title="WAL & Checkpoints" icon={Gauge} accentColor={THEME.info || THEME.primary}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
