...
const generateSparklinePoints = (data) => {
  ...
  const points = data.map((val, i) => {
    const x = (i / (len - 1 || 1)) * (width - 2 * padding) + padding;
...
const getStatusColor = (status) => {
  return status === 'healthy' ? DARK_THEME.success :
    status === 'warning' ? DARK_THEME.warning :
    DARK_THEME.danger;
};

const generateRandomSparkline = (seed) => {
  let val = seed;
...
function KPICard({ label, value, unit, status, sparkline }) {
  const statusColor = getStatusColor(status);
  const points = generateSparklinePoints(sparkline);

  return (
    <div style={{
      background: `linear-gradient(135deg, ${DARK_THEME.card} 0%, rgba(22, 27, 34, 0.8) 100%)`,
      ...
      position: 'relative',
      overflow: 'hidden',
      boxShadow: `0 8px 16px rgba(0, 0, 0, 0.3), inset 0 1px 0 ${DARK_THEME.border}`,
    }}>
      ...
      {/* Sparkline */}
      <svg width="100%" height="24" style={{ marginTop: 4, position: 'relative', zIndex: 1 }}>
        <polyline
          points={points}
          fill="none"
          stroke={statusColor}
